...
export default function Users() {
  ...
  const roles = ['all', 'Médecin', 'Administrateur', 'Secrétaire']
        ...
        <main className="p-6">
          ...
          <div className="grid grid-cols-4 gap-4 mb-6">
            <Card className="flex items-center gap-4">
              ...
              <div>
                <p className="text-2xl font-bold text-[#1D2D35]">{users.length}</p>
                <p className="text-sm text-[#5E7480]">Total</p>
              </div>
            </Card>
            {['Médecin', 'Administrateur', 'Secrétaire'].map((role) => {
              const Icon = roleIcons[role]
              const count = users.filter(u => u.role === role).length
              return (
                <Card key={role} className="flex items-center gap-4">
                  <div className={cn('w-12 h-12 rounded-xl bg-gradient-to-br flex items-center justify-center', roleColors[role])}>
                    <Icon size={22} className="text-white" />
                  </div>
                  <div>
                    <p className="text-2xl font-bold text-[#1D2D35]">{count}</p>
                    <p className="text-sm text-[#5E7480]">{role}s</p>
                  </div>
                </Card>
              )
            })}
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 text-sm text-[#5E7480] mb-2">
                        <RoleIcon size={14} />
                        <span>{user.role} - {user.specialty}</span>
                      </div>
